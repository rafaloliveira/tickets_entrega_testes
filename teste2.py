        ...
        if focal_atual:
            ...
            for i in range(0, len(tickets_focal), num_colunas):
                ...
                for j, row in enumerate(tickets_focal[i:i+num_colunas]):
                    with linha[j]:
                        # Dentro do loop for j, row in enumerate(tickets_focal[i:i+num_colunas]):

                        ticket_id = row["id"]
                        safe_idx = ticket_id  # para session_state keys, pode usar id diretamente

                        # Inicializa session_state para datas e complemento manual
                        data_abertura_manual_key = f"data_abertura_manual_{safe_idx}"
                        hora_abertura_manual_key = f"hora_abertura_manual_{safe_idx}"
                        data_finalizacao_manual_key = f"data_finalizacao_manual_{safe_idx}"
                        hora_finalizacao_manual_key = f"hora_finalizacao_manual_{safe_idx}"
                        complementar_key = f"complementar_final_{safe_idx}"

                        if data_abertura_manual_key not in st.session_state:
                            try:
                                abertura_str = row.get("data_hora_abertura") or row.get("abertura_timestamp")
                                dt_abertura = parser.parse(abertura_str.replace('T', ' '))
                                st.session_state[data_abertura_manual_key] = dt_abertura.date()
                                st.session_state[hora_abertura_manual_key] = dt_abertura.time()
                            except:
                                st.session_state[data_abertura_manual_key] = date.today()
                                st.session_state[hora_abertura_manual_key] = datetime.now().time()
                        ...
                        if complementar_key not in st.session_state:
                            st.session_state[complementar_key] = ""

                        with st.expander("🔒 Finalizar Ocorrência"):
                            complemento = st.text_area("Complementar (obrigatório)", key=complementar_key)
                            ...
                            if st.button("✅ Finalizar", key=f"finalizar_btn_{safe_idx}"):
                                if not complemento.strip():
                                    ...
                                    try:
                                        ...
                                        abertura_str = row.get("abertura_timestamp") or row.get("data_hora_abertura")
                                        abertura_dt = parser.parse(abertura_str.replace('T', ' ')).replace(tzinfo=None)

                                        delta = dt_finalizacao - abertura_dt
                                        horas = str(delta.seconds // 3600).zfill(2)
                                        minutos = str((delta.seconds // 60) % 60).zfill(2)
                                        segundos = str(delta.seconds % 60).zfill(2)
                                        permanencia_manual = f"{horas}:{minutos}:{segundos}"
