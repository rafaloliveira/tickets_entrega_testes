                                    ...
                                    try:
                                        ...
                                        if data_hora_finalizacao < data_hora_abertura:
                                            ...
                                            try:
                                                ...
                                                enviar_em# versão liberada para usuario
...
def classificar_ocorrencia_por_tempo(data_abertura_input):
    ...
    try:
        ...
        data_abertura = datetime.strptime(
            f"{ocorr['data_abertura_manual']} {ocorr['hora_abertura_manual']}",
            "%Y-%m-%d %H:%M:%S"
        ).replace(tzinfo=pytz.UTC).astimezone(tz_sp)
    except Exception as e:
        print(f"Erro ao converter data: {e}")
        return "Erro", "gray"
        ...
        for ocorr in ocorrencias_abertas:
            try:
                if (
                    ...
                    tz_sp = pytz.timezone("America/Sao_Paulo")
                    data_abertura = parser.parse(f"{ocorr['data_abertura_manual']} {ocorr['hora_abertura_manual']}")
                    agora = datetime.now(tz_sp)
                    if data_abertura.tzinfo is None:
                        data_abertura = tz_sp.localize(data_abertura)
                    else:
                        data_abertura = data_abertura.astimezone(tz_sp)
                    gora = datetime.now(tz_sp)
                    delta = agora - data_abertura  # agora e data_abertura já estão com tz_sp
        ...
        for idx, ocorr in enumerate(ocorrencias_abertas):
            ...
            with colunas[idx % num_colunas]:
                ...
                with st.expander("Finalizar Ocorrência"):
                    ...
                    if st.button("Finalizar", key=f"finalizar_{safe_idx}", disabled=finalizar_disabled):
                        try:
                            data_hora_finalizacao = parser.parse(f"{data_finalizacao_manual} {hora_finalizacao_manual}")
                            if data_hora_finalizacao.tzinfo is None:
                                data_hora_finalizacao = tz_sp.localize(data_hora_finalizacao)
                            else:
                                data_hora_finalizacao = data_hora_finalizacao.astimezone(tz_sp)

                            data_hora_abertura = parser.parse(f"{ocorr['data_abertura_manual']} {ocorr['hora_abertura_manual']}")
                            if data_hora_abertura.tzinfo is None:
                                data_hora_abertura = tz_sp.localize(data_hora_abertura)
                            else:
                                data_hora_abertura = data_hora_abertura.astimezone(tz_sp)
                            ...
                            if st.button("Finalizar", key=finalizar_btn_key):
                                if not complemento.strip():
                                    ...
                                    try:
                                        ...
                                        st.error(f"❌ Erro ao finalizar ocorrência: {e}")ail_ocorrencia(
                                                    cliente_nome=ocorr.get("cliente"),
                                                    assunto=assunto_finalizacao,
                                                    corpo_email=corpo_finalizacao
                                                )
