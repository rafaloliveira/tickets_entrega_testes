...
def main():
    ...
    #st.sidebar.info("Navegação feita automaticamente com arquivos em /pages")
    st.title("📦 Pré Roterização")

    df = carregar_base_supabase()
    if df is not None:
        # Remove entregas já confirmadas
        ...
        if not dados_confirmados.empty and "Serie_Numero_CTRC" in dados_confirmados.columns and "Serie/Numero CTRC" in df.columns:
            # Padroniza as colunas para evitar problemas de tipo ou espaços
        ...
        # FILTRO DE ROTA (SELECTBOX)
        rotas_disponiveis = sorted(df["Rota"].dropna().unique())
        rotas_opcoes = ["Todes"] + rotas_disponiveis
        rota_selecionada = st.selectbox("🔎 Filtrar por Rota:", rotas_opcoes)

        if rota_selecionada != "Todas":
            df = df[df["Rota"] == rota_selecionada]

        # RESUMO TOTAL DE ROTAS E ENTREGAS
        ...
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
            <div style="background-color: #2e2e2e; padding: 16px 24px; border-radius: 8px; color: white; text-align: center; display: inline-block;">
        ...
        for rota in rotas_unicas:
            ...
            colunas_exibidas = [
                'Indice', 'Serie/Numero CTRC', 'Cliente Pagador', 'Chave CT-e', 'Cliente Destinatario',
            ...
            if selecionadas is not None and len(selecionadas) > 0:
                st.success(f"{len(selecionadas)} entregas selecionadas nesta rota.")
                
                # Cria DataFrame e faz renomeação imediatamente
                ...
                if st.button(f"✅ Confirmar Rota: {rota}", key=f"confirmar_{rota}"):
                    try:
                        # Define colunas a serem inseridas no Supabase
                        colunas_para_salvar = [
                        ...
                        df_selecionadas = df_selecionadas[[col for col in colunas_para_salvar if col in df_selecionadas.columns]]

                        registros_para_inserir = df_selecionadas.to_dict(orient="records")
                        supabase.table("rotas_confirmadas").insert(registros_para_inserir).execute()
                        st.success("Entregas confirmadas com sucesso!")
                        st.rerun()

                    except Exception as e:
                        st.error(f"Erro ao confirmar rota: {e}")
